...
if args.list:
    p = subprocess.Popen(["cd ../terraform/stage; terraform show | grep external_ip_address_app; cd -"], stdout=subprocess.PIPE, shell=True)
    appip=p.stdout.read().split('"')[1]

    p = subprocess.Popen(["cd ../terraform/stage; terraform show | grep external_ip_address_db; cd -"], stdout=subprocess.PIPE, shell=True)
